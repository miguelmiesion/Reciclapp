...
class QrScanViewModel(private val repository: WasteRepository) : ViewModel(){
    ...
    fun claimWaste(resultString: String) {
        ...
        viewModelScope.launch {
            _uiState.update { it.copy(isLoading = true, error = false, success = false, message = null) }
                try {
                    ...
                    when (val result = repository.claimWaste(wasteId)) {
                        is NetworkResult.Success -> {
                            _uiState.update { it.copy(success = true, isLoading = false, message = "Sumaste $points puntos!") }
                        }
                        is NetworkResult.Error -> {
                            _uiState.update { it.copy(isLoading = false, error = true, message = result.message ?: "Error en el servidor, intente nuevamente.") }
                        }
                    }
                } catch (e: Exception) {
                    Log.e("CLAIMWASTE", e.message ?: "ERROR SIN MENSAJE")
                    _uiState.update { it.copy(isLoading = false, error = true, message =  "Error en el servidor, intente nuevamente.") }
                }
        }
    }
